...
#This is docker file based on the jboss eap 7.4 image from Azure VM ( 133 ) machine
...
COPY jboss-eap-7.4.zip $JBOSS_HOME
...
# install JBoss EAP 7.4.0
ADD jboss-eap-7.4.zip /tmp/jboss-eap-7.4.zip
RUN unzip /tmp/jboss-eap-7.4.zip

# set environment
ENV JBOSS_HOME /opt/eap/jboss-eap-7.4
ENV APP=$APPNAME
